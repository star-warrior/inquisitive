...
export function clearAnimatedCards() {
...
export default function ResourceCard({
  ...
  const {
    ...
    transition,
    isDragging,
  } = useSortable({ id: resource.id, disabled: isOverlay });
  ...
  return (
    ...
    >
      <motion.div
        className={cn(
          "w-full flex items-stretch bg-white border rounded-[var(--radius-card)] overflow-hidden shadow-sm group/card transition-colors transition-shadow duration-300 p-[var(--spacing-card-p)] gap-3.5 select-none",
          isDragging
